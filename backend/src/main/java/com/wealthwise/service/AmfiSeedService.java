...
public class AmfiSeedService {
    ...
    /**
     * Fetches the LATEST NAV for a single scheme directly from MFAPI.
     * Called on-demand by HoldingsService when no local NAV data is found.
     * Saves the result into nav_daily and updates scheme_master.last_nav
     * so subsequent requests use the cached value.
     *
     * API: GET https://api.mfapi.in/mf/{schemeCode}/latest
     * Response: {"meta":{...}, "data":[{"date":"18-04-2026","nav":"104.5956"}]}
     *
     * @return the fetched NAV, or null if unavailable
     */
    public BigDecimal fetchLatestNavForScheme(String schemeCode) {
        String url = "https://api.mfapi.in/mf/" + schemeCode + "/latest";
        log.info("[NAV-Live] Fetching live NAV for scheme={} from MFAPI", schemeCode);
        try {
            ...
            if (conn.getResponseCode() != 200) {
                log.warn("[NAV-Live] MFAPI returned HTTP {} for scheme={}", conn.getResponseCode(), schemeCode);
                return null;
            }

            // Read entire JSON response into a string
            ...
            // Format: {"meta":{...},"data":[{"date":"18-04-2026","nav":"104.5956"}]}
            BigDecimal navValue = null;
            LocalDate  navDate  = null;

            // Extract first "date" value
            int dateIdx = json.indexOf("\"date\":");
            if (dateIdx >= 0) {
                int s = json.indexOf('"', dateIdx + 7) + 1;
                int e = json.indexOf('"', s);
                if (s > 0 && e > s) {
                    String raw = json.substring(s, e);  // e.g. "18-04-2026"
                    try {
                        navDate = LocalDate.parse(raw,
                            DateTimeFormatter.ofPattern(raw.contains("-") && raw.length() == 10
                                && raw.charAt(2) == '-' && raw.charAt(5) == '-'
                                ? "dd-MM-yyyy" : "dd-MMM-yyyy"));
                    } catch (Exception ex) {
                        try { navDate = LocalDate.parse(raw, DateTimeFormatter.ofPattern("dd-MMM-yyyy")); }
                        catch (Exception ex2) { log.warn("[NAV-Live] Could not parse date '{}'", raw); }
                    }
                }
            }

            // Extract first "nav" value
            int navIdx = json.indexOf("\"nav\":");
            if (navIdx >= 0) {
                int s = json.indexOf('"', navIdx + 6) + 1;
                int e = json.indexOf('"', s);
                if (s > 0 && e > s) {
                    try { navValue = new BigDecimal(json.substring(s, e).trim()); }
                    catch (Exception ex) { log.warn("[NAV-Live] Could not parse NAV for scheme={}", schemeCode); }
                }
            }

            if (navValue == null || navValue.compareTo(BigDecimal.ZERO) <= 0) {
                log.warn("[NAV-Live] No valid NAV in MFAPI response for scheme={}", schemeCode);
                return null;
            }

            // ── Persist to nav_daily ──────────────────────────────────────────
            // Wrapped in its own try-catch: concurrent requests may both try to
            // INSERT the same (scheme_code, nav_date) — the loser just ignores
            // the duplicate key error and moves on. We still have navValue.
            if (navDate != null) {
                try {
                    if (!navRepo.existsBySchemeCodeAndNavDate(schemeCode, navDate)) {
                        NavDaily nd = new NavDaily();
                        nd.setSchemeCode(schemeCode);
                        nd.setNavDate(navDate);
                        nd.setNavValue(navValue);
                        nd.setCreatedAt(OffsetDateTime.now());
                        navRepo.save(nd);
                    ...
                    // Duplicate key from concurrent thread — record is already saved, ignore
                    log.debug("[NAV-Live] nav_daily already saved by another thread for scheme={} date={}",
                        schemeCode, navDate);
                }
            }

            // ── Update scheme_master.last_nav ─────────────────────────────────
            final BigDecimal finalNav  = navValue;
            final LocalDate  finalDate = navDate;
            try {
                schemeRepo.findByAmfiCode(schemeCode).ifPresent(sm -> {
                    if (finalNav.compareTo(BigDecimal.ZERO) > 0) {
                        sm.setLastNav(finalNav);
                        if (finalDate != null) sm.setLastNavDate(finalDate);
                        sm.setUpdatedAt(OffsetDateTime.now());
                        schemeRepo.save(sm);
                    }
                });
            } catch (Exception smEx) {
                log.debug("[NAV-Live] scheme_master update skipped for scheme={}: {}", schemeCode, smEx.getMessage());
            }

            log.info("[NAV-Live] ✔ scheme={} nav={} date={}", schemeCode, navValue, navDate);
            return navValue;   // ← always return the value we fetched from MFAPI

        } catch (Exception e) {
            // Only reaches here if the HTTP fetch itself failed (network error etc.)
            log.warn("[NAV-Live] Failed to fetch NAV for scheme={}: {}", schemeCode, e.getMessage());
            return null;
        }
    }
